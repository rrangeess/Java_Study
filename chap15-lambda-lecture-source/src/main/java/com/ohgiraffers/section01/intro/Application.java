...
//            Calculator cal = new Calculator()
        ...
        /* 목차. 1. 인터페이스를 구현하는 클래스로 하위 구현체를 만드는 방법 (feat. 자식 클래스 필요)*/
            Calculator cal = new CalculatorImpl();   // 다형성을 적용해 실행
        System.out.println("10과 20의 합은 ?" + cal.sumTwoNumbers(10, 20));  // 동적 바인딩 실행

        /* 목차. 2. 익명클래스를 활용한 방식(인터페이스 타입으로 하위 구현체 생성 = 자식(구현) 클래스 없애기 */
        ...
        *   유일하게 있는 추상메소드를 오버라이딩 하는 개념이기 때문에 익명 함수가 가능하다다.
        ...
        *   3. 실행 될 마지막 구문이 리터럴 형태로 끝나고 추상메소드에 반환값이 있을 경우 return이
        *     생략 가능하다.
        *
        * */
