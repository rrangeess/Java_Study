...
//        StringBuffer sb2 = new StringBuffer("javamariaDB");   //메소드 차이는 없지만 동시성 제어를 위한 작업으로 속도가 다소 느림
                                                                //코테 볼때 StringBuilder 쓰기
        /* 설명.
        *   delete(): 시작 인데스와 종료 인덱스를 사용해서 문자열에서 원하는 문자열 부분 제거
        *   deleteCharAt(): 문자열 인덱스를 이용해서 문자 하나를 제거한다.
        * */
        System.out.println("delete() : " + sb.delete(2,5));    //string + string 형식으로 된다.
        System.out.println("deleteCharAt(): " + sb.deleteCharAt(2));

        /* 설명.
        *   insert(): 인자로 전달된 값을 문자열로 변환 후 지정된 인덱스 위치에 추가한다.
        * */
        System.out.println("insert() : " + sb.insert(1,"vao"));   //offset은 기본 위치
