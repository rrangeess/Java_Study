...
public class Application {
    public static void main(String[] args) {
        ...
        *  설명.
        *   장점
        *   1. 첫번쨰 이용시에는 인스턴스를 생성해야 하므로 속도 차이가 나지 않지만
        *      두번째 이후 이용시에는 인스턴스 생성 시간 없이 바로 사용할 수 있다.(feat.재사용)
        *   2. 인스턴스가 절대적으로 한개만 할당 되는 것을 보증할 수 있다.
        *   단점
        *   1. 싱글톤 인스턴스가 너무 많은 일을 하거나 많은 데이터를 공유하면 결합도가 높아진다.
        *   2. 동시성 문제를 고려해서 설계해야 하기 때문에 난이도가 높다. (멀티 스레드 환경과 밀접)
        *
        *  설명.
        *   구현 방법
        ...
        * */
        /* 설명. EagerSingleton은 프로그램 시작시 바로 객체가 할당 */
        EagerSingleton eager1 = EagerSingleton.getInstance();               //프로그램이 시작하자마자 객체 생성
        ...
        LazySingleton lazy1 = LazySingleton.getInstance();                  //LazySingleton은 최초 호출시 객체가 할당
